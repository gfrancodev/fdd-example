...
export class TaskHttpService implements Task.Service {
  ...
  private async initializeOnboarding(): Promise<void> {
    const onboarding = this.onboardingStorage.getJSON(
      TaskHttpService.ONBOARDING_KEY,
    );
  ...
  async createTaskGroup(name: string): Promise<Task.Group> {
    ...
    this.storage.setJSON(
      TaskHttpService.TASK_GROUPS_KEY,
      updatedGroups,
      this.DEFAULT_STORAGE_OPTIONS as any,
    );
  ...
  async addTask(groupId: string, text: string): Promise<Task.Root> {
    ...
    const updatedGroups = this.updateGroupInList(
      groups,
      groupIndex,
      updatedGroup,
    );
  ...
  async getSelectedTaskGroupId(): Promise<string | undefined> {
    await delay(200);
    return (
      this.selectedGroupStorage.get(TaskHttpService.SELECTED_GROUP_ID_KEY) ||
      undefined
    );
  }
  ...
  async closeOnboarding(): Promise<void> {
    ...
    const updatedOnboarding = {
      ...
      hasCompletedOnboarding:
        onboarding.completed.length === TaskHttpService.STEPS.length,
    };

    this.onboardingStorage.setJSON(
      TaskHttpService.ONBOARDING_KEY,
      updatedOnboarding,
      this.DEFAULT_STORAGE_OPTIONS as any,
    );
  ...
  async completeOnboardingStep(step: string): Promise<void> {
    ...
    const hasCompletedOnboarding =
      completed.length === TaskHttpService.STEPS.length;
  ...
  private determineNextStep(currentStep: string): string {
    ...
    if (
      currentIndex === -1 ||
      currentIndex >= TaskHttpService.STEPS.length - 1
    ) {
      return currentStep;
    }
  ...
  private updateGroupInList(
    groups: Task.Group[],
    index: number,
    updatedGroup: Task.Group,
  ): Task.Group[] {
    return [
